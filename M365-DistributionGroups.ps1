...
Get-DistributionGroup -Filter {{MemberDepartRestriction -eq "Closed"} -or {MemberJoinRestriction -eq "Closed"}} | Where-Object {$_.GroupType -notmatch "SecurityEnabled"};
...
Get-DistributionGroup -Filter {{MemberDepartRestriction -eq "Closed"} -or {MemberJoinRestriction -eq "Closed"}} | `
	Where-Object {$_.GroupType -notmatch "SecurityEnabled"} | Select-Object Primary*,DisplayName,*Restriction,GroupType,WhenCreated | `
	Where-Object {Set-DistributionGroup -Identity $_.PrimarySmtpAddress -MemberJoinRestriction $dg_join -MemberDepartRestriction $dg_depart};
